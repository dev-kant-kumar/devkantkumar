...
const Settings = () => {
  ...
  // Settings hooks
  const { data: genSettings, isLoading: isLoadingGen } =
    useGetAdminGeneralSettingsQuery();
  const { data: seoSettings, isLoading: isLoadingSeo } =
    useGetAdminSeoSettingsQuery();
  const [updateGen] = useUpdateAdminGeneralSettingsMutation();
  const [updateSeo] = useUpdateAdminSeoSettingsMutation();
  ...
  const [initiateEmailChange, { isLoading: isInitiatingEmail }] =
    useInitiateEmailChangeMutation();
  const [verifyEmailChangeOTP, { isLoading: isVerifyingEmail }] =
    useVerifyEmailChangeOTPMutation();
  const [initiatePasswordChange, { isLoading: isInitiatingPasswordChange }] =
    useInitiatePasswordChangeMutation();
  const [verifyPasswordChangeOTP, { isLoading: isVerifyingPasswordChange }] =
    useVerifyPasswordChangeOTPMutation();

  // 2FA Hooks
  const [setup2FA, { isLoading: isSettingUp2FA }] = useSetupAdmin2FAMutation();
  const [verify2FASetup, { isLoading: isVerifying2FA }] =
    useVerifyAdmin2FASetupMutation();
  const [disable2FA, { isLoading: isDisabling2FA }] =
    useDisableAdmin2FAMutation();
                ...
                <div className="flex flex-col sm:flex-row items-center gap-6 mb-8 group text-center sm:text-left">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      Manage your avatar and identity.
                    </p>
                  </div>
                </div>
                  ...
                  <div className="flex flex-col gap-6">
                    <div className="space-y-2">
                      ...
                      <input
                        ...
                        className="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white/50 dark:bg-gray-800/50 text-gray-900 dark:text-white"
                      />
                    </div>
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-gray-400 uppercase tracking-widest">
                          ...
                          <div className="flex items-center justify-between pointer-events-none">
                            <div className="flex gap-2">
                              {[1, 2, 3].map((step) => (
                                <div
                                  key={step}
                                  className={`w-8 h-1 rounded-full transition-colors ${emailVerificationStep >= step ? "bg-blue-600" : "bg-gray-200 dark:bg-gray-700"}`}
                                />
                              ))}
                            </div>
                            <button
                              onClick={clearEmailState}
                          ...
                          {emailVerificationStep === 1 && (
                            <div className="space-y-3">
                              ...
                              <button
                                ...
                                disabled={
                                  isVerifyingEmail || emailOtp.some((d) => !d)
                                }
                    ...
                    <div className="space-y-2">
                      ...
                      <textarea
                        ...
                        onChange={handleInputChange}
                        rows={4}
                        className="w-full p-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-white/50 dark:bg-gray-800/50 text-gray-900 dark:text-white resize-none"
                      />
                    </div>
                  </div>
                ...
                <form onSubmit={onSubmitSite} className="space-y-8">
                      ...
                      <div className="space-y-4">
                        <div className="space-y-2">
                          ...
                          <input
                            ...
                            onChange={handleSiteInputChange}
                            className="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white/50 dark:bg-gray-800/50 text-gray-900 dark:text-white font-medium"
                          />
                        </div>
                        <div className="space-y-2">
                          <label className="text-xs font-bold text-gray-400 uppercase tracking-widest">
                          ...
                          <textarea
                            ...
                            onChange={handleSiteInputChange}
                            rows={3}
                            className="w-full p-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-white/50 dark:bg-gray-800/50 text-gray-900 dark:text-white text-xs resize-none"
                          />
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="p-6 bg-blue-50 dark:bg-blue-900/10 rounded-2xl border border-blue-100 dark:border-blue-800 flex gap-4">
                    ...
                    <div className="text-xs text-blue-700 dark:text-blue-300 leading-relaxed">
                      ...
                      These global variables affect the entire portfolio
                      application, including meta tags for social sharing
                      (OpenGraph) and the main site header identity.
                    </div>
                  </div>
                      ...
                      <div className="p-4 bg-green-50 dark:bg-green-900/10 rounded-xl border border-green-200 dark:border-green-800 flex items-start justify-between gap-4">
                        <div>
                          ...
                          <p className="text-xs text-green-600 dark:text-green-400 mt-1">
                            Your account is secured with two-factor
                            authentication.
                          </p>
                        </div>
                        <button
                      ...
                      <div className="p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row items-center justify-between gap-4">
                        <div>
                          ...
                          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                            Protect your account using an authenticator app like
                      ...
                      >
                        <div className="p-6 space-y-6">
                          ...
                          <div className="flex flex-col items-center gap-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-xl">
                            ...
                            <p className="text-xs text-center text-gray-500 font-mono break-all">
                              {twoFactorSecret}
                            </p>
                          </div>
                  ...
                  {passwordVerificationStep === 1 && (
                    ...
                    >
                      ...
                      <p className="text-xs text-gray-600 dark:text-gray-400">
                        ...
                        to confirm this password change.
                      </p>
                      <div className="flex gap-2 justify-center">
                        {passwordOtp.map((digit, idx) => (
                          <input
                            ...
                            value={digit}
                            onChange={(e) =>
                              handlePasswordOtpChange(idx, e.target.value)
                            }
                            onKeyDown={(e) => {
                              if (e.key === "Backspace" && !digit && idx > 0) {
                                document
                                  .getElementById(`pwd-otp-${idx - 1}`)
                                  .focus();
                              }
                            }}
                            className="w-10 h-12 text-center text-lg font-bold rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:border-blue-500 outline-none"
                      ...
                      <div className="text-center">
                        {passwordOtpTimer > 0 ? (
                          <p className="text-xs text-gray-400">
                            Resend code in {passwordOtpTimer}s
                          </p>
                        ) : (
                          <button
                            type="button"
