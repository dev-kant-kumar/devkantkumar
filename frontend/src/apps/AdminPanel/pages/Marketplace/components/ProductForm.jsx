...
import { useUploadFilesMutation, useUploadImageMutation } from '../../../store/api/adminApiSlice';
...
const InputField = ({ label, name, value, onChange, placeholder, type = "text", icon: Icon, error, ...props }) => (
    <div className="relative group">
        ...
        <div className="relative">
            ...
            <input
                ...
                value={value}
                onChange={onChange}
                className={`w-full ${Icon ? 'pl-10' : 'pl-4'} pr-4 py-3 rounded-xl bg-gray-50 dark:bg-gray-800/50 border transition-all duration-200 outline-none
                    ${error
        ...
        {error && <p className="text-red-500 text-xs mt-1 ml-1 animate-in slide-in-from-top-1">{error}</p>}
    </div>
);
    ...
    return (
        <div className="relative group" ref={dropdownRef}>
            ...
            >
                ...
                <div className={`absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`}>
                    <ChevronDown size={18} />
                </div>
            </button>
...
const ProductForm = ({ initialData, onSubmit, isLoading, onCancel }) => {
  ...
  const [regionalInput, setRegionalInput] = useState({ currency: 'USD', price: '' });
  ...
  const validate = () => {
    ...
    if (formData.sourceCodeUrl.trim() && !urlPattern.test(formData.sourceCodeUrl)) {
  ...
  const handleFileUpload = async (e) => {
      ...
      const ALLOWED_EXTS = ['.zip', '.rar', '.pdf', '.docx', '.pptx', '.txt', '.png', '.jpg', '.jpeg', '.svg', '.webp'];
      ...
      try {
          ...
          if (res.files) {
              setFormData(prev => ({
                  ...prev,
                  downloadFiles: [...(prev.downloadFiles || []), ...res.files.map(f => ({
                      name: f.originalName,
                      url: f.url,
                      size: f.size,
                      type: f.mimetype
  ...
  const handleAddRegionalPrice = () => {
      if (!regionalInput.price || Number(regionalInput.price) <= 0) {
          toast.error('Enter a valid price');
          return;
      }
      if (formData.regionalPricing.some(p => p.currency === regionalInput.currency)) {
          toast.error(`${regionalInput.currency} price already exists`);
          return;
      }
      ...
      setFormData(prev => ({
          ...
          regionalPricing: [
              ...
              { currency: regionalInput.currency, price: Number(regionalInput.price) }
          ]
      }));
      setRegionalInput(prev => ({ ...prev, price: '' }));
  };

  const removeRegionalPrice = (currency) => {
      setFormData(prev => ({
          ...prev,
          regionalPricing: prev.regionalPricing.filter(p => p.currency !== currency)
      }));
  };
  ...
  const handleAddExternalFile = () => {
    ...
    setFormData(prev => ({
        ...prev,
        downloadFiles: [...prev.downloadFiles, {
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      if (res.image) {
        setFormData(prev => ({
          ...prev,
          images: [...prev.images, { url: res.image.url, public_id: res.image.id }]
        }));
  ...
  const handleSubmit = (e) => {
    ...
    const submissionData = {
      ...
      regionalPricing: formData.regionalPricing.map(p => ({
          ...
          region: p.currency === 'USD' ? 'US' : 'GLOBAL'
      }))
    };
    ...
    <form onSubmit={handleSubmit} className="space-y-10">
      ...
      <section className="space-y-6">
          ...
          <div className="relative group">
              <div className="flex justify-between mb-1.5 ml-1">
                  ...
                  <span className={`text-xs ${formData.description.length >= MAX_DESC_CHARS ? 'text-red-500' : 'text-gray-400'}`}>
          ...
          <div className="relative group">
              ...
              <div className={`w-full min-h-[50px] px-3 py-2 rounded-xl bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-500/20 transition-all flex flex-wrap gap-2`}>
                  {formData.tags.map((tag, idx) => (
                       <span key={idx} className="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800 animate-in zoom-in-50 duration-200">
                            {tag}
                            <button
                                type="button"
                                onClick={(e) => {
                  ...
                  <input
                      ...
                      placeholder={formData.tags.length < MAX_TAGS ? "Type & Enter..." : "Max tags reached"}
              ...
              <p className="text-xs text-gray-400 mt-1 ml-1">Type tag and press Enter</p>
           </div>
      </section>
      ...
      <section className="space-y-6">
           <div className="flex items-center gap-2 pb-2 border-b border-gray-100 dark:border-gray-800">
              ...
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Start Features</h3>
          </div>
          ...
          <div className="space-y-4">
              ...
              <div className="space-y-2">
                  {formData.features.map((feature, idx) => (
                      <div key={idx} className="flex items-center justify-between p-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl group hover:border-indigo-500/50 transition-colors">
      ...
      <section className="space-y-6">
          ...
          <div className="space-y-4">
              <label className="block text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 ml-1">
                  Product Images <span className="text-gray-400 font-normal normal-case ml-1">(Cover & Gallery)</span>
              </label>

              {formData.images.length > 0 && (
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {formData.images.map((img, idx) => (
                          <div key={idx} className="group relative aspect-video rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-800">
              ...
              <label className={`relative flex flex-col items-center justify-center w-full h-32 border-2 border-dashed rounded-xl cursor-pointer transition-all duration-200
                  ${errors.images
                  ...
                  <div className="flex flex-col items-center justify-center pt-5 pb-6 text-gray-500 dark:text-gray-400">
                      {isUploading ? <Loader className="w-8 h-8 animate-spin mb-2" /> : <Upload className="w-8 h-8 mb-2" />}
                      <p className="text-sm font-medium">Click to upload or drag & drop</p>
                      <p className="text-xs opacity-70 mt-1">PNG, JPG, WEBP (Max 100MB)</p>
                  </div>
                  <input type="file" accept="image/*" onChange={handleImageUpload} className="hidden" disabled={isUploading} />
              </label>
              {errors.images && <p className="text-red-500 text-xs ml-1">{errors.images}</p>}
          </div>

          <div className="space-y-4">
              <label className="block text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 ml-1">
                  Product Files <span className="text-gray-400 font-normal normal-case ml-1">(Downloadable Assets)</span>
              </label>

              <div className="columns-2 md:columns-3 gap-4 space-y-4 pb-4">
                  {formData.downloadFiles.map((file, idx) => (
                      <div key={idx} className="break-inside-avoid relative group rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 overflow-hidden mb-4 shadow-sm hover:shadow-md transition-all">
                          {/* Preview Area */}
                          {file.type?.toString().startsWith('image/') ? (
                              <div className="relative">
                                  <img src={file.url} alt="" className="w-full h-auto block" />
                              ...
                              <div className="p-8 flex flex-col items-center justify-center bg-gray-50 dark:bg-gray-800/50 aspect-[4/3] group-hover:bg-gray-100 dark:group-hover:bg-gray-800 transition-colors relative">
                                  <FileText size={40} className="text-gray-400 mb-3 group-hover:text-blue-500 transition-colors" />
                                  <span className="text-xs text-gray-400 uppercase tracking-wider font-semibold">{file.type || 'FILE'}</span>
                          ...
                          <div className="p-3 border-t border-gray-100 dark:border-gray-800">
                              <p className="text-sm font-medium text-gray-900 dark:text-white truncate mb-1" title={file.name}>
                                  {file.name}
                              </p>
                              <div className="flex items-center justify-between">
                                  ...
                                  <span className="text-[10px] text-gray-400 uppercase bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded">
                                      {file.type ? file.type.split('/').pop().toUpperCase() : 'LINK'}
                                  </span>
                              </div>
                          </div>
                      </div>
              ...
              <label className={`relative flex flex-col items-center justify-center w-full h-24 border-2 border-dashed rounded-xl cursor-pointer transition-all duration-200
                  ...
                  <div className="flex flex-col items-center justify-center pt-4 pb-5 text-gray-500 dark:text-gray-400">
                      {isUploadingFiles ? <Loader className="w-6 h-6 animate-spin mb-2" /> : <Upload className="w-6 h-6 mb-2" />}
                      <p className="text-sm font-medium">Upload Product Files (ZIP, PDF, etc)</p>
                  </div>
                  <input type="file" multiple onChange={handleFileUpload} className="hidden" disabled={isUploadingFiles} />
              </label>
              {errors.downloadFiles && <p className="text-red-500 text-xs ml-1">{errors.downloadFiles}</p>}

              <div className="flex items-center gap-4 py-2">
                  <div className="h-px bg-gray-200 dark:bg-gray-700 flex-1"></div>
                  <span className="text-xs text-gray-500 font-medium uppercase">OR ADD LINK</span>
                  <div className="h-px bg-gray-200 dark:bg-gray-700 flex-1"></div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                    <input
                        ...
                        onChange={(e) => setExternalFile(prev => ({ ...prev, name: e.target.value }))}
                    ...
                    <input
                        ...
                        onChange={(e) => setExternalFile(prev => ({ ...prev, url: e.target.value }))}
      ...
      <section className="space-y-6">
           <div className="flex items-center gap-2 pb-2 border-b border-gray-100 dark:border-gray-800">
              ...
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Global Pricing</h3>
          </div>
          ...
          <div className="bg-blue-50/50 dark:bg-blue-900/10 p-4 rounded-xl flex items-start gap-3 border border-blue-100 dark:border-blue-900/30">
              ...
              <p className="text-sm text-blue-800 dark:text-blue-200">
                  <strong>Important:</strong> Pricing is displayed to users based on their location.
                  It is recommended to set prices for all major currencies to ensure accurate global billing.
                  The default INR price will be used as a fallback.
              </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
               {/* Base Price */}
               <div>
                   <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-4 opacity-70">Region: INDIA (Default)</h4>
                   <InputField
               ...
               <div className="space-y-4">
                  <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-4 opacity-70">Region: INTERNATIONAL</h4>

                  {/* Currency Adder */}
                  <div className="flex items-end gap-3">
                      <div className="w-1/2">
                          <label className="block text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-1.5 ml-1">Currency</label>
                          <div className="relative">
                            <select
                                value={regionalInput.currency}
                                onChange={(e) => setRegionalInput(prev => ({ ...prev, currency: e.target.value }))}
                                className="w-full px-4 py-3 rounded-xl bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 text-gray-900 dark:text-white appearance-none outline-none"
                            >
                                {CURRENCIES.filter(c => !formData.regionalPricing.find(rp => rp.currency === c.code)).map(c => (
                      ...
                      <div className="w-1/2">
                          <label className="block text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-1.5 ml-1">Price</label>
                          <input
                              type="number"
                              value={regionalInput.price}
                              onChange={(e) => setRegionalInput(prev => ({ ...prev, price: e.target.value }))}
                              placeholder="0.00"
                              className="w-full px-4 py-3 rounded-xl bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 text-gray-900 dark:text-white outline-none"
                          />
                      </div>
                      <button
                          type="button"
                          onClick={handleAddRegionalPrice}
                          className="px-4 py-3 bg-gray-900 dark:bg-gray-700 text-white rounded-xl font-medium hover:bg-black dark:hover:bg-gray-600 transition-colors shadow-sm"
                      >
                          <Plus size={20} />
                      </button>
                  </div>
                  ...
                  <div className="space-y-2">
                      {formData.regionalPricing.length === 0 ? (
                           <div className="text-center py-6 border-2 border-dashed border-gray-100 dark:border-gray-800 rounded-xl">
                               <p className="text-xs text-gray-400">No international prices set.</p>
                           </div>
                      ) : (
                          formData.regionalPricing.map((rp) => (
                              <div key={rp.currency} className="flex items-center justify-between px-4 py-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/50 rounded-xl shadow-sm hover:shadow-md transition-shadow">
                                  <div className="flex items-center gap-3">
                                      <div className="w-8 h-8 rounded-full bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center text-emerald-700 dark:text-emerald-400 font-bold text-xs">
                                          {rp.currency.substring(0, 1)}
                                      </div>
                                      <div>
                                          <p className="text-sm font-semibold text-gray-900 dark:text-white">{rp.currency}</p>
                                          <p className="text-xs text-gray-500">Region: {rp.currency === 'USD' ? 'US' : 'GLOBAL'}</p>
                                      </div>
                                  </div>
                                  <div className="flex items-center gap-4">
                                      <span className="text-gray-900 dark:text-white font-mono font-medium">{rp.price}</span>
                                      <button
                                          type="button"
                                          onClick={() => removeRegionalPrice(rp.currency)}
