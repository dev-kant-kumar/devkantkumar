...
const TicketDetail = () => {
  ...
  const {
    ...
    error,
    refetch,
  } = useGetAdminSupportTicketByIdQuery(id, {
    pollingInterval: 30000,
  });
  const [respondToTicket, { isLoading: isResponding }] =
    useRespondToAdminSupportTicketMutation();
  ...
  }, [ticket?.responses?.length, scrollToBottom]);

  // Socket: join/leave ticket room for real-time
  useEffect(() => {
    if (!socket || !id) return;

    emit("support:join", { ticketId: id });
    ...
    const handleTyping = (data) => {
      if (data.ticketId === id && data.sender === "user") {
    ...
    return () => {
      emit("support:leave", { ticketId: id });
      socket.off("support:message", handleNewMessage);
      socket.off("support:typing", handleTyping);
      if (typingTimeoutRef.current) clearTimeout(typingTimeoutRef.current);
    };
  }, [socket, id, emit, refetch]);

  // Emit typing indicator when admin types
  const handleReplyChange = (e) => {
    setReplyMessage(e.target.value);
    if (socket && id) {
      emit("support:typing", { ticketId: id, sender: "admin" });
    }
  };
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              <span
                className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-tighter border ${statusColor} flex items-center gap-1.5`}
              >
            ...
            {ticket.responses && ticket.responses.length > 0 ? (
              <div className="space-y-8">
                {ticket.responses.map((response, idx) => (
                  ...
                  >
                    ...
                    <div
                      className={`flex items-center gap-3 mt-3 px-4 text-[10px] font-bold ${response.sender === "admin" ? "text-blue-500" : "text-gray-500"}`}
                    >
                      ...
                      {response.sender === "admin" && (
                        <>
                          <span className="opacity-20">•</span>
                          <span
                            className={`${response.isRead ? "text-green-400" : "text-gray-600"}`}
                          >
                            {response.isRead ? "✓✓ Read" : "✓ Sent"}
                          </span>
                        </>
                      )}
                    </div>
                ...
                {isUserTyping && (
                  ...
                  >
                    <div className="bg-gray-800/80 backdrop-blur-md border border-white/5 px-6 py-4 rounded-[2rem] rounded-tl-none">
              ...
              <div className="bg-white/5 backdrop-blur-sm rounded-[2.5rem] p-16 text-center border-2 border-dashed border-white/10">
                ...
                <p className="text-gray-500 mt-2 max-w-sm mx-auto">
                  Click below to start a professional conversation with this
                  customer.
                </p>
              </div>
            )}
        ...
        <div className="space-y-8">
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between p-4 bg-white/5 border border-white/5 rounded-2xl">
                ...
                <span
                  className={`text-sm font-black uppercase tracking-tighter ${PRIORITY_CONFIG[ticket.priority]?.color || "text-white"}`}
                >
          ...
          <div className="bg-gradient-to-br from-blue-600 to-indigo-800 rounded-[2rem] p-8 text-white shadow-2xl shadow-blue-500/10 relative overflow-hidden group border border-white/10">
            ...
            <p className="text-sm text-blue-100 leading-relaxed font-medium">
              Follow professional communication standards. Aim for a resolution
              within 4 business hours for high-priority tickets.
            </p>
          </div>
        </div>
      </div>
