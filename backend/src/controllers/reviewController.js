...
exports.createReview = catchAsync(async (req, res, next) => {
  ...
  let { product, service, user } = req.body;
  if (product) product = String(product);
  if (service) service = String(service);

  if (!product && !service) {
    return next(
      new AppError("Review must belong to a product or service.", 400),
    );
  }
  ...
  if (orders.length === 0) {
    ...
    return next(
      new AppError("You can only review items you have purchased.", 403),
    );
  }
